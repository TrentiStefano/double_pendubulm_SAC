        ...
        if self.state_representation == 2:
            x = np.array(
                [
                    ...
                    (observation[1] * np.pi + np.pi) % (2 * np.pi) - np.pi,  # [-pi, pi]
                    observation[2] * self.max_velocity,  # From [-1, 1] -> [-max_velocity, max_velocity]
    ...
    def normalize_state(self, state):
        ...
        if self.state_representation == 2:
            observation = np.array(
                [
                    (state[0] % (2 * np.pi) - np.pi) / np.pi, # From [0, 2pi] -> [-1, 1]
                    ((state[1]+np.pi )% (2 * np.pi) - np.pi) / np.pi, # From [-pi, pi] -> [-1, 1]
